...
import { callContract } from '@/services/wallet';  
...
// Updated interface with description field  
...
// Updated createCircle function with description  
export async function createCircle(params: CreateCircleParams): Promise<TransactionResult> {    
  ...
  const payoutIntervalBlocks = params.payoutIntervalDays * 144;    
  const decayRate = (params.reputationDecayRate || 1) * 100;    
  const decayInterval = (params.decayIntervalDays || 30) * 144;    
  ...
  return callContract(    
    ...
    [    
      ...
      stringAsciiCV(params.description || ''),  // Added description  
      uintCV(contributionMicroSTX),    
      uintCV(params.maxMembers),    
      uintCV(payoutIntervalBlocks),    
      uintCV(decayRate),    
      uintCV(decayInterval),    
    ]    
  );    
}    
    
// Add function to trigger decay processing    
export async function processReputationDecay(circleId: number): Promise<TransactionResult> {    
  return callContract(    
...
// Fixed isDecayDue function  
export async function isDecayDue(circleId: number): Promise<boolean> {    
  const config = getContractConfig();    
  const stacksNetwork = getNetwork();    
      
  try {    
    const result = await callReadOnlyFunction({    
      contractAddress: config.address,    
      contractName: config.coreContract,    
      functionName: 'get-circle',  // Fixed: use existing function  
    ...
    const circle = json.value?.value;  
      
    if (!circle) return false;  
      
    const currentBlock = await getCurrentBlockHeight();  
    const lastDecayBlock = circle['last-decay-block']?.value || 0;  
    const decayInterval = circle['decay-interval']?.value || 0;  
      
    return (currentBlock - lastDecayBlock) >= decayInterval;  
  } catch (error) {    
...
async function getCurrentBlockHeight(): Promise<number> {  
  ...
  try {  
    const result = await callReadOnlyFunction({  
      contractAddress: config.address,  
      contractName: config.coreContract,  
      functionName: 'get-current-block',  
      functionArgs: [],  
      senderAddress: config.address,  
      network: stacksNetwork,  
    });  
